...
Choose format:
    ...
    await ctx.reply(`❌ Error analyzing YouTube video: ${error.message}`);
    ...
    await ctx.replyWithMarkdown(caption, {
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: `📥 Download ${mediaInfo.isVideo ? "Video" : "Photo"}`,
              callback_data: `instagram_${Buffer.from(url).toString("base64")}`,
            },
          ],
        ],
      },
...
*Duration:* ${videoInfo.duration}
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "📥 Download Video",
                callback_data: `tiktok_${Buffer.from(url).toString("base64")}`,
              },
            ],
          ],
        },
    ...
    await ctx.reply(
      "❌ Error analyzing TikTok content. Please check the URL and try again."
    );
      ...
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: `📥 Download ${mediaInfo.isVideo ? "Video" : "Photo"}`,
              callback_data: `twitter_${Buffer.from(url).toString("base64")}`,
            },
          ],
        ],
      },
...
bot.on("callback_query", async (ctx) => {
  ...
  const action = parts[1]; // for youtube: video or audio
  const encodedUrl = parts.slice(2).join("_"); // in case url has _
...
bot
  ...
  .then(() => {
    ...
    console.log("📋 Supported platforms: YouTube, Instagram, TikTok, Twitter");
  })
  .catch((err) => {
